...
function pickBucketIndex(balanceXrp: number): number {
  for (let i = 0; i < BUCKETS.length; i++) {
    const b = BUCKETS[i];
    const within = balanceXrp >= b.min && balanceXrp < (b.max === Infinity ? Number.POSITIVE_INFINITY : b.max);
    if (within) return i;
  }
...
export async function fetchDistributionFromLedger(options?: {
  ...
  const rpcUrl = options?.rpcUrl || process.env.XRPL_RPC_URL || 'https://s2.ripple.com:51234/';
