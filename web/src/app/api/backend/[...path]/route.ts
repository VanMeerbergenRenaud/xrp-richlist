...
// Envs
const XRPSCAN_BASE = process.env.XRPSCAN_BASE_URL || 'https://api.xrpscan.com/api/v1';
...
async function proxyFetch(req: NextRequest, targetUrl: string) {
  ...
  outHeaders.set('content-type', outHeaders.get('content-type') || 'application/json');

  const data = await res.arrayBuffer();
  return new NextResponse(data, { status: res.status, statusText: res.statusText, headers: outHeaders });
}
